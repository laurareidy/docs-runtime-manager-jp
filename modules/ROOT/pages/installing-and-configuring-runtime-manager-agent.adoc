...
既存する以前の Runtime Manager エージェントインストールを更新できます。新しい Runtime Manager エージェントをインストールする方法については、下の「<<インストールオプション>>」セクションを参照してください。
...
Runtime Manager エージェントでプロキシサーバを設定するのに加えて、`wrapper.conf` ファイルでもプロキシサーバを設定して API 関連の通信と分析も有効にする必要があります。詳細は、「<<プロキシのセットアップ>>」を参照してください。
...
`--secure`
|Runtime Manager エージェントがオンプレミス管理コンソールと TLS 接続を確立するように設定します。トラストストアとキーストアを JKS 形式で指定する必要があります。このオプションでは、REST 通信用の TLS チャネルのみが有効になります。<<セキュア接続チャネル>>を参照してください。これは、手動でエージェントを管理するためのものです (ARM クラウドコンソールを使用せずにエージェントを管理する場合など)。
