...
Runtime Manager エージェントは Mule Runtime Engine にインストールされ、Runtime Manager に Mule を登録するために使用されます。Mule Runtime Engine は特定の Anypoint Platform ビジネスグループの特定の環境内の Runtime Manager に登録され、この Runtime Manager によって制御されます。

Mule Runtime Engine の最新バージョンは、Runtime Manager エージェントの最新バージョンにバンドルされています。最新の Mule Runtime Engine は、 https://www.mulesoft.com/support-login[カスタマーポータル]からダウンロードできます。

[NOTE]
使用している Anypoint Platform のバージョンをサポートしている Runtime Manager エージェントのバージョンをダウンロードしてインストールする必要があります。必要な Runtime Manager エージェントのバージョンは、Runtime Manager の最新バージョンにバンドルされているバージョンとは異なる可能性があります。使用しているプラットフォームのバージョンをサポートするために、エージェントのバージョンをダウングレードする必要がある場合もあります。

Mule Runtime Engine または API ゲートウェイの以前のバージョンを使用している場合や、Mule Runtime Engine のリリースの間に Runtime Manager エージェントの新しいバージョンがリリースされた場合は、Runtime Manager エージェントの最新バージョンを https://www.mulesoft.com/support-login[カスタマーポータル]からダウンロードしてインストールできます。
...
[TIP]
====
Mule Enterprise の最新バージョンを実行することをお勧めします。最新のバージョンは、 http://www.mulesoft.com/support-login[カスタマーポータル]からダウンロードできます。
====
...
エージェントをダウンロードし、<<Installing the Agent from the ZIP File,「ZIP ファイルからのエージェントのインストール」>>セクションの手順に従います。そして、OS に応じて `amc_setup` または `amc_setup.bat` を実行します。
====
...
既存する以前の Runtime Manager エージェントインストールを更新できます。新しい Runtime Manager エージェントをインストールする方法については、下の「<<Installation Options>>」セクションを参照してください。
...
`amc_setup` コマンドを使用して定義できない設定もあります。たとえば、JMX 監視を他の外部サービスに拡張する場合などです。このようなオプションは、手動で `$MULE_HOME/conf/mule-agent.yml` ファイルに追加する必要があります。また、`$MULE_HOME/conf/mule-agent.yml` ファイルでは、さまざまな設定オプションをバックアップできます。 
...
|接続に登録するときに使用するプロキシ設定。このオプションによってプロキシの詳細を定義します。<<Installation Via Proxy>>を参照してください。
...
[WARNING]
====
複数の Runtime Manager ビジネスグループまたは環境への Mule Runtime Engine の登録はサポートされていません。

また、古い xref:mule-management-console::index.adoc[Mule Management Console (MMC)] と Runtime Manager の両方に Mule Runtime Engine を登録することもサポートされていません。Mule Runtime Engine が現在 MMC で管理されている場合は、`amc_setup -H` スクリプトを実行する前に、Mule Runtime Engine の登録を解除する必要があります。
====
...
Runtime Manager エージェントでプロキシサーバを設定するのに加えて、`wrapper.conf` ファイルでもプロキシサーバを設定して API 関連の通信と分析も有効にする必要があります。詳細は、「<<Setting up a Proxy>>」を参照してください。
...
`--insecure`
|Runtime Manager エージェントが暗号化されていない接続を使用するように設定します。これは、REST トランスポートでのみ有効です。ブラウザや、HTTP 要求を実行するその他のツールを使用して、API とのやりとりを行うことができます。デフォルトの TCP ポートは 9999 です。そのため、ベース URL `+https://localhost:9999/mule/agent/+` で Runtime Manager エージェントに接続できます。
...
インストールコマンドで -S 引数を指定して設定できるこのオプションでは、Runtime Manager エージェントがオンプレミス管理コンソールと TLS 接続を確立するように設定します。

トラストストアとキーストアを JKS 形式で指定する必要があります。このオプションでは、REST 通信用の TLS チャネルのみが有効になります。セキュア接続チャネルモードを選択すると、次のメニューが表示されます。
...
`--secure`
|Runtime Manager エージェントがオンプレミス管理コンソールと TLS 接続を確立するように設定します。トラストストアとキーストアを JKS 形式で指定する必要があります。このオプションでは、REST 通信用の TLS チャネルのみが有効になります。<<Secure Connection Channel>>を参照してください。これは、手動でエージェントを管理するためのものです (ARM クラウドコンソールを使用せずにエージェントを管理する場合など)。
...
----
_Keystore location (キーストアの場所)_

通信チャネルを暗号化するためのキーストアファイルの場所。キーストアは JKS 形式である必要があります。必ず指定する必要があります。

_Truststore location (トラストストアの場所)_

管理コンソールからの要求を受信するためのトラストストアの場所。トラストストアは JKS 形式である必要があります。またパスワードは設定しないでください。

_Keystore Password (キーストアパスワード)_

キーストアを参照するためのパスワード。このパスワードは、エージェントがキーストアを開くために使用されます。

_Keystore Alias (キーストアの別名)_

キーストア内に保存された鍵の別名。

_Keystore Alias Password (キーストアの別名のパスワード)_
...
=== 双方向 TLS 用の Mule Runtime の設定

`amc_setup -S` オプションを使用して双方向 TLS を設定する例を次に示します。

TLS を設定する手順は、次のとおりです。

. Runtime Manager エージェント (サーバ) を識別するキーストア (公開/非公開鍵ペア) を生成します。Runtime Manager エージェントのホスト名または IP アドレスに一致する CN を設定します。
...
. これらは自己署名証明書ファイルであり、クライアントが認証機関 (CA) を使用してファイルを検証できないため、クライアントと RMA の両方の証明書 (公開鍵) を含むトラストストアファイルを作成して、CA によるこれらの両方の証明書への署名をエミュレートする必要があります。本番環境では、サーバとクライアントの両方の証明書は信頼された CA によって署名され、クライアントマシンとサーバマシンに対して公開または共有されます。
...
. `rmakeystore.jks` ファイルと `cacerts.jks` トラストストアを使用して Mule Runtime Engine を設定します。`$MULE_HOME/bin` フォルダからコマンド `./amc_setup -S` を実行します。たとえば、これまでのコマンドをすべて `/security` フォルダから実行した場合は、次の値を入力します。
...
----
`/security/cacerts.jks` トラストストアファイルが `$MULE_HOME/conf` フォルダにインポートされ、名前が `anypoint-truststore.jks` に変更されます。 +
[NOTE]
デフォルトのトラストストアは、Runtime Manager エージェントのバージョン 1.10.0 以降、名前が `anypoint-truststore.jks` に変更されました。Runtime Manager エージェントの以前のバージョンでは、デフォルトのトラストストアは `truststore.jks` という名前でした。 

. Mule Runtime Engine を再起動し、Runtime Manager エージェントの REST インターフェースが正常に起動したことを確認します。Mule Runtime Engine のログに SSL デバッグを追加します。 `./mule -M-Djavax.net.debug=all`
