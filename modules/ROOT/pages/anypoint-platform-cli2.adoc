...
* xref:1.x@api-manager::index.adoc[API Manager 1.x (Crowd プレリリース)]
* xref:design-center::index.adoc[Design Center]

== 前提条件

Anypoint-CLI は、NodeJS アプリケーションです。Anypoint Platform CLI をコンピュータにインストールするには、NPM (デフォルトの Node.js パッケージマネージャ) と共に NodeJS ランタイム環境がコンピュータにインストールされていることを確認する必要があります。
NodeJS のドキュメントに従って、両方のリソースを取得します。

また、git バージョン管理システムもコンピュータにインストールする必要があります。特定のオペレーティングシステムのインストール手順やダウンロードリンクは、Git ドキュメントサイトにあります。

NodeJS (npm を使用) と git をインストールしたら、コマンドラインインターフェースを開いて以下を入力し、Anypoint Platform CLI をインストールできます。
...
|host (ホスト) | Anypoint Platform インストールのホスト。 +
...
|bearer (ベアラー) | ユーザ名とパスワードを渡す代わりに、アカウントのトークンを渡すことができる。 +
...
これらの使用方法の手順については、xref:anypoint-platform-cli2-commands.adoc[「Anypoint Platform CLI 2.x のコマンドリスト」]を参照してください。
...
* <<ローカル PCF デプロイ済みアプリケーションを管理するコマンド>>
* <<ローカルサーバを管理するコマンド>>
...
|===
|コマンド |説明
|<<runtime-mgr pcf-application delete>> | PCF アプリケーションを削除する
|<<runtime-mgr pcf-application describe>> | PCF アプリケーションの詳細情報を表示する
|<<runtime-mgr pcf-application list>> | 環境内のすべての PCF アプリケーションをリストする
|<<runtime-mgr pcf-application restart>> | PCF アプリケーションを再起動する
|<<runtime-mgr pcf-application stop>> | PCF アプリケーションを停止する
|<<runtime-mgr pcf-application deploy>> | 新しいアプリケーションを PCF 領域にデプロイする
|<<runtime-mgr pcf-application describe-json>> | PCF アプリケーションの未加工の JSON 応答を表示する
|<<runtime-mgr pcf-application modify>> | PCF 領域にデプロイされているアプリケーションを編集する
|<<runtime-mgr pcf-application start>> | PCF アプリケーションを開始する
|===
...
Anypoint-CLI コールの形式は、次のようになります。
[source,console]
----
$ anypoint-cli [params] [command]
----

コマンドを渡さない場合、Anypoint-CLI は​*対話型モード*​で実行されます。
...
----
このコマンドは、Anypoint Platform のすべての環境をリストします。環境名、その ID、および Sandbox 環境かどうかどうかを返します。 +
このコマンドでは、デフォルトのオプション (`--help`、`-f`/`--fields`、`-o`/`--output`) 以外のオプションは使用できません。
...
このコマンドは、API 参照名、API の最新バージョン、API のバージョン数、最後に更新されてからの経過時間をリストします。
...
引数「source」と「target」は、([]/)<>/<version> の形式にする必要があります。 +
...
`api-1` という名前の API (バージョン `1.0`) の定義を _サービス_ 組織から _開発_ 組織にコピーします。 +
...
`api-1` という名前の API (バージョン `1.0`) のポリシーを _サービス_ 組織から _開発_ 組織にコピーします。
...
`api-1` という名前の API (バージョン `1.0`) の API Portal を _サービス_ 組織から _開発_ 組織にコピーします。 +
Anypoint Platform CLI でサービス組織を使用する場合、コマンドの `source` としてアプリケーション名を使用できます。
...
`api-1` という名前の API (バージョン `1.0`) の層を _サービス_ 組織から _開発_ 組織にコピーします。 +
Anypoint Platform CLI でサービス組織を使用する場合、コマンドの `source` としてアプリケーション名を使用できます。
...
プロジェクトの名前の入力を開始して `tab` キーを押すと、Anypoint-CLI によって名前がオートコンプリートされます。また、`tab` キーをダブルタップすると、渡すことができるすべての値の完全なリストが表示されます。 +
...
引数「assetIdentifier」は、<>/<>/<version> の形式にする必要があります。
...
このコマンドは、Anypoint-CLI で使用できるすべてのアプリケーションをリストします。アプリケーション、その状況、割り当てられている vCore の数、最終更新時間を返します。 +
このコマンドでは、デフォルトのオプション (`--help`、`-f`/`--fields`、`-o`/`--output`) 以外のオプションは使用できません。
...
アプリケーションの名前の入力を開始して `tab` キーを押すと、Anypoint-CLI によって名前がオートコンプリートされます。また、`tab` キーをダブルタップすると、渡すことができるすべての値の完全なリストが表示されます。 +
...
|--help                                                  | 使用量情報を出力する
|===
Anypoint-CLI から静的 IP を割り当てることはできません。有効化または無効化のみが可能です。
...
また、オプションの入力を開始して `tab` キーを押し、Anypoint-CLI でオプションをオートコンプリートすることもできます。
...
----
`application-1` という名前のアプリケーションを _サービス_ 組織の QA 環境から _開発_ 組織の QA 環境にコピーします。 +
...
----
`application-1` という名前のアプリケーションを _サービス_ 組織の QA 環境からサーバ ID 123456 の _開発_ 組織の QA 環境にコピーします。 +
Anypoint Platform CLI でサービス組織の QA 環境を使用する場合、コマンドの `source` としてアプリケーション名を使用できます。
...
このコマンドは、`appId` で渡される PCF アプリケーションを削除します。 +
このコマンドでは、デフォルトのオプション (`--help`、`-f`/`--fields`、`-o`/`--output`) 以外のオプションは使用できません。
...
このコマンドは、`appId` で渡される PCF デプロイ済みアプリケーションの情報を表示します。 +
このコマンドでは、デフォルトのオプション (`--help`、`-f`/`--fields`、`-o`/`--output`) 以外のオプションは使用できません。
...
このコマンドは、すべての PCF デプロイ済みアプリケーションをリストします。 +
このコマンドでは、デフォルトのオプション (`--help`、`-f`/`--fields`、`-o`/`--output`) 以外のオプションは使用できません。
...
このコマンドは、`appId` で渡される PCF デプロイ済みアプリケーションを再起動します。 +
このコマンドでは、デフォルトのオプション (`--help`、`-f`/`--fields`、`-o`/`--output`) 以外のオプションは使用できません。
...
このコマンドは、`appId` で渡される PCF デプロイ済みアプリケーションを停止します。 +
このコマンドでは、デフォルトのオプション (`--help`、`-f`/`--fields`、`-o`/`--output`) 以外のオプションは使用できません。
...
このコマンドは、`name` で渡される名前 ID を使用して、`zipfile` で渡されるアプリケーションを PCF インスタンスにデプロイします。 +
このコマンドでは、デフォルトの `--help`、`-f`/`--fields`、`-o`/`--output` オプション以外に次のオプションも使用できます。
...
このコマンドは、`appId` で渡される PCF デプロイ済みアプリケーションの未加工の JSON の情報を表示します。 +
このコマンドでは、デフォルトのオプション (`--help`、`-f`/`--fields`、`-o`/`--output`) 以外のオプションは使用できません。
...
このコマンドは、`zipfile` で渡される ZIP ファイルアプリケーションを使用して、`appId` で渡される PCF デプロイ済みアプリケーションを変更します。
...
このコマンドは、`appId` で渡される PCF デプロイ済みアプリケーションを開始します。 +
このコマンドでは、デフォルトのオプション (`--help`、`-f`/`--fields`、`-o`/`--output`) 以外のオプションは使用できません。
...
ロードバランサの名前の入力を開始して `tab` キーを押すと、Anypoint-CLI によって名前がオートコンプリートされます。また、`tab` キーをダブルタップすると、渡すことができるすべての値の完全なリストが表示されます。 +
...
| name (名前) |ロードバランサの名前。 | newtestloadbalancer
| Certificate (証明書) |ローカルハードドライブのサーバ証明書の `.pem` ファイルへの絶対パス。 +
...
|値 |説明 |例
| name (名前) |ロードバランサの名前。 | newtestloadbalancer
| Certificate (証明書) |ローカルハードドライブの証明書の `.pem` ファイルへの絶対パス。 +
*証明書ファイルは、暗号化せずに PEM でエンコードする必要があります。* | /Users/mule/Documents/cert.pem
...
|===
|値 |説明 |例
|name (名前) |ルールが適用されるロードバランサの名前。 |testloadbalancer
|index |ルールの優先度。  |1
...
VPC の名前の入力を開始して `tab` キーを押すと、Anypoint-CLI によって名前がオートコンプリートされます。また、`tab` キーをダブルタップすると、渡すことができるすべての値の完全なリストが表示されます。 +
このコマンドでは、デフォルトのオプション (`--help`、`-f`/`--fields`、`-o`/`--output`) 以外のオプションは使用できません。
...
|===
|オプション |説明
| サーバ | 特殊なドメインを解決する DNS サーバの IP。最大 3 回指定できます。
| ドメイン | 特殊な DNS サーバリストで解決するドメイン。複数回指定できます。
|===
...
> exit [options]
----
このコマンドは、Anypoint-CLI を終了します。 +
このコマンドでは、デフォルトのオプション (`--help`、`-f`/`--fields`、`-o`/`--output`) 以外のオプションは使用できません。Anypoint Platform には、以下を操作するコマンドラインツールが用意されています。