...
新しい外部メッセージハンドラを追加するには、Runtime Manager エージェントの<<Reverse String External Message Handler>>インターフェースを実装する必要があります。
...
新しい内部メッセージハンドラを追加するには、<<Internal Message Handler>>インターフェースを実装する必要があります。
